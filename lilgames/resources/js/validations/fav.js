...
const show = (gamesfav) => {
    gamesfav = gamesfav.gamesfav;
            ...
            gamesfav.forEach((game) => {
                let card = $('<div class="card col-5 col-md m-2 mx-auto border-white text-center bg-prpl">');
                ...
                card.append(svg)
                card.append('<a href="/catalog/'+game.nombreJuego+'"><img class="card-img-top juegos align-self-center img-fluid" src="'+baseUrl+'/'+game.nombreJuego+'.png" alt="'+game.nombreJuego+' Logo"/><div class="card-body bg-prpl"><h4 class="card-title titulos fs-5 text-white">'+game.nombreJuego+'</h4><p class="card-text textos d-none d-md-block">'+game.info+'</p></div></a></div>');
                content.append(card);
            });
}
$(document).ready(()=> {
